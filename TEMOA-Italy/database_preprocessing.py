            ...
            if i < len(Efficiency_i.vintage) - 1:  # Interpolation
                ...
                for j in range(0, len(time_periods_i)):
                    if j == 0:  # Only used for the first time period available (to avoid / 0 in the linear interpolation equation)
                        ...
                        eff_units.append(Efficiency_i.eff_units[i])
    ...
    Efficiency = pd.DataFrame(
        {
            ...
            "eff_units": pd.Series(eff_units, dtype='str'),
        ...
        if Demand.periods[i] == base_year:
            ...
            for j in range(0, len(Allocation.demand_comm)):
                if Allocation.demand_comm[j] == Demand.demand_comm[i]:
                    # FIX: pre-filter Driver by (driver_name, regions) and sort by
                    # period so that Driver_filt.driver[k-1] is the previous period
                    # of the SAME driver_name × region (not the previous row in the
                    # unfiltered Driver table, which may belong to a different
                    # driver_name or region and produce a meaningless growth rate).
                    Driver_filt = Driver[
                        (Driver.driver_name == Allocation.driver_name[j]) &
                        (Driver.regions == Demand.regions[i])
                    ].sort_values(by='periods').reset_index(drop=True)
                    for k in range(0, len(Driver_filt.periods)):
                        for l in range(0, len(Elasticity.periods)):
                            # FIX: add region filter on Elasticity so that base-year
                            # demand for region R only spawns projection rows in
                            # region R (not in every region that happens to have an
                            # elasticity row for this commodity).
                            if (Elasticity.demand_comm[l] == Demand.demand_comm[i]
                                    and Elasticity.regions[l] == Demand.regions[i]
                                    and Driver_filt.periods[k] == Elasticity.periods[l]):
                                regions.append(Elasticity.regions[l])
                                periods.append(int(Elasticity.periods[l]))
                                demand_comm.append(Elasticity.demand_comm[l])
                                if not Driver_filt.periods[k] == base_year:
                                    demand.append(float(np.format_float_scientific(demand[len(demand) - 1] * (1 + (Driver_filt.driver[k] / Driver_filt.driver[k - 1] - 1) * Elasticity.elasticity[l]))))
